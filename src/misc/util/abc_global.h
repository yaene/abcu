...
#pragma warning( \
        disable : 4244)  // warning C4244: '+=' : conversion from 'int ' to
                         // 'unsigned short ', possible loss of data
#pragma warning(disable : 4514)  // warning C4514: 'Vec_StrPop' : unreferenced
                                 // inline function has been removed
#pragma warning( \
        disable : 4710)  // warning C4710: function 'Vec_PtrGrow' not inlined
...
#define ABC_PRTP(a, t, T)                                    \
  (Abc_Print(1, "%s =", (a)),                                \
   Abc_Print(1, "%9.2f sec (%6.2f %%)\n",                    \
             1.0 * ((double)(t)) / ((double)CLOCKS_PER_SEC), \
             ((double)(T)) ? 100.0 * ((double)(t)) / ((double)(T)) : 0.0))
#define ABC_PRM(a, f)         \
  (Abc_Print(1, "%s =", (a)), \
   Abc_Print(1, "%10.3f MB\n", 1.0 * ((double)(f)) / (1 << 20)))
#define ABC_PRMr(a, f)        \
  (Abc_Print(1, "%s =", (a)), \
   Abc_Print(1, "%10.3f MB\r", 1.0 * ((double)(f)) / (1 << 20)))
#define ABC_PRMn(a, f)        \
  (Abc_Print(1, "%s =", (a)), \
   Abc_Print(1, "%10.3f MB  ", 1.0 * ((double)(f)) / (1 << 20)))
#define ABC_PRMP(a, f, F)                                                    \
  (Abc_Print(1, "%s =", (a)),                                                \
   Abc_Print(1, "%10.3f MB (%6.2f %%)\n", (1.0 * ((double)(f)) / (1 << 20)), \
             (((double)(F)) ? 100.0 * ((double)(f)) / ((double)(F)) : 0.0)))

#ifdef ABC_USE_CUDA
#define ABC_ALLOC(type, num)                                       \
  ({                                                               \
    type *_p_;                                                     \
    cudaMallocManaged((void **)&_p_, sizeof(type) * (size_t)(num), \
                      cudaMemAttachGlobal);                        \
    _p_;                                                           \
  })
...
#define ABC_FALLOC(type, num)                                 \
  ((type *)memset(malloc(sizeof(type) * (size_t)(num)), 0xff, \
                  sizeof(type) * (size_t)(num)))
...
static inline char *Abc_UtilStrsav(char *s) {
  return s ? strcpy(ABC_ALLOC(char, strlen(s) + 1), s) : NULL;
}
...
static inline int Abc_BitByteNum(int nBits) {
  return (nBits >> 3) + ((nBits & 7) > 0);
}
static inline int Abc_BitWordNum(int nBits) {
  return (nBits >> 5) + ((nBits & 31) > 0);
}
static inline int Abc_Bit6WordNum(int nBits) {
  return (nBits >> 6) + ((nBits & 63) > 0);
}
static inline int Abc_TruthByteNum(int nVars) {
  return nVars <= 3 ? 1 : (1 << (nVars - 3));
}
static inline int Abc_TruthWordNum(int nVars) {
  return nVars <= 5 ? 1 : (1 << (nVars - 5));
}
static inline int Abc_Truth6WordNum(int nVars) {
  return nVars <= 6 ? 1 : (1 << (nVars - 6));
}
static inline int Abc_InfoHasBit(unsigned *p, int i) {
  return (p[(i) >> 5] & (unsigned)(1 << ((i)&31))) > 0;
}
static inline void Abc_InfoSetBit(unsigned *p, int i) {
  p[(i) >> 5] |= (unsigned)(1 << ((i)&31));
}
static inline void Abc_InfoXorBit(unsigned *p, int i) {
  p[(i) >> 5] ^= (unsigned)(1 << ((i)&31));
}
static inline unsigned Abc_InfoMask(int nVar) {
...
#define APPLE_MACH 0
  ...
  struct timespec ts;
  if (clock_gettime(CLOCK_MONOTONIC, &ts) < 0) return (abctime)-1;
...
#if (defined(LIN) || defined(LIN64)) && !APPLE_MACH && !defined(__MINGW32__)
  struct timespec ts;
  if (clock_gettime(CLOCK_THREAD_CPUTIME_ID, &ts) < 0) return (abctime)-1;
  abctime res = ((abctime)ts.tv_sec) * CLOCKS_PER_SEC;
  res += (((abctime)ts.tv_nsec) * CLOCKS_PER_SEC) / 1000000000;
  return res;
#else
...
static inline void Abc_Print(int level, const char *format, ...) {
  ...
  if (!Abc_FrameIsBridgeMode()) {
    ...
    if (level == ABC_ERROR)
      Gia_ManToBridgeText(stdout, (int)strlen("Error: "),
                          (unsigned char *)"Error: ");
    else if (level == ABC_WARNING)
      Gia_ManToBridgeText(stdout, (int)strlen("Warning: "),
...
static inline void Abc_PrintTimeP(int level, const char *pStr, abctime time,
                                  abctime Time) {
  ABC_PRTP(pStr, time, Time);
}

static inline void Abc_PrintMemoryP(int level, const char *pStr, int mem,
                                    int Mem) {
...
static inline void *Abc_FileReadContents(char *pFileName, int *pnFileSize) {
  ...
  if (pnFileSize) *pnFileSize = nFileSize;
  rewind(pFile);
...
static inline void Abc_ReverseOrder(int *pA, int nA) {
  int i;
  for (i = 0; i < nA / 2; i++) ABC_SWAP(int, pA[i], pA[nA - 1 - i]);
}
...
extern void Abc_QuickSortCostData(int *pCosts, int nSize, int fDecrease,
                                  word *pData, int *pResult);
